...
//============================================================================================================================
    ...
    public function marker_select_historical_heartrate_data_from_heartrate_table(Request $request, Response $response, $args)
    {
      try{
        if (isset($_POST['USN'])||isset($_POST['historical_interval'])) {
          $user['USN'] =  $_POST['USN'];
    ...
    public function app_select_historical_heartrate_data_from_heartrate_table(Request $request, Response $response, $args)
    {
      header('Content-type:application/json');
      $json = file_get_contents('php://input'); //allows the server to read raw POST data from the request body.
      $data = json_decode($json, true);
      ...
      try{
        if (isset($data['USN'])||isset($data['historical_interval'])) {
          $user['USN'] =  $data['USN'];
          $user['historical_interval'] =  $data['historical_interval'];
        } else {
          $json = ['error_message' => 'There is no value.', 'result_code' => 1];
